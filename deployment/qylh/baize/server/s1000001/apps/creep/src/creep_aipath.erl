...
%% 寻路
find(Actor, Dest, SceneSt = #scene_st{scene=SceneID}) ->
    case scene_path_stupid:find(SceneID, Actor#actor.coord, Dest) of
        {ok, []}   ->
            ?SUCCESS;
        {ok, Path} ->
...
towards_finding(Coord1, Coord2, Offset, SceneSt) ->
    ...
    case scene_util:walkable(SceneSt#scene_st.scene, Coord3) of
        true  -> Coord3;
        false -> Coord2
    end.
