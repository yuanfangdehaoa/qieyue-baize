...
# 停服、备份
playbook merge_prepare2.yml ${Host} \
	-e servers=${Servers} \
...
for Server in ${ServerArr}; do
    ...
    BackupFile=${DIR_BACKUP}/server-${Platform}-${ServerID}-lastest.tar.gz
...
## 创建目标机器的合服数据目录
playbook merge_before.yml ${Host} \
  -e servers=${LastServer} \
  -e suid_from=${ServerID} \
  -e dir_backup=${DIR_BACKUP}


# 开始合服
playbook merge_server2.yml ${Host} \
	-e servers=${LastServer} \
	-e suids=${SUIDs} \
	-e suid_from=${ServerID}
