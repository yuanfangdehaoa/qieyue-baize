...
-export([merge/1]).
-export([post/0]).
-export([dir_merge/0, dir_merge/1]).
...
merge(SUIDs) ->
	...
	Fun =
		fun(#r_tab{name = Tab}) ->
			[ets:new(?ETS_TAB(Tab, SUID), [public, named_table, set, {keypos, 2}]) || SUID <- SUIDs]
		end,
	...
	lists:foreach(fun
									(#r_tab{rec=Tab}) ->
										case lists:member(Tab, clear_tabs()) of
											...
											false ->
												Data  = get_data(SUID, Tab),
												Data2 = merge_game_tab2(Tab, Data),
												lists:foreach(fun
																				(Rec) ->
...
merge_game_tab2(game_user, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:map(fun
							(User1 = #game_user{id=UserID, roles=RoleIDs0}) ->
								...
								case db:dirty_read(?DB_GAME_USER, UserID) of
									...
									[] ->
										User1#game_user{roles=RoleIDs1}
								end
						end, Data);
merge_game_tab2(game_rank, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:map(fun
							(Rank1 = #game_rank{id=RankID, ranklist=RankList0, alldata=AllData0}) ->
								...
								case db:dirty_read(?DB_GAME_RANK, RankID) of
									...
									[] ->
										Rank1#game_rank{ranklist=RankList1, alldata=AllData1}
								end
						end, Data);
merge_game_tab2(mailbox, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filter(fun
								 (Mailbox) ->
									 not maps:is_key(Mailbox#mailbox.owner, Del)
							 end, Data);
merge_game_tab2(friend, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filtermap(fun
										(Friend) ->
											case maps:is_key(Friend#friend.id, Del) of
												true  ->
													false;
												false ->
													#friend{applied=Applied, roles=Roles} = Friend,
...
merge_game_tab2(chat_contact, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filtermap(fun
										(Contact) ->
											case maps:is_key(Contact#chat_contact.id, Del) of
												true  ->
													false;
												false ->
													{true, Contact#chat_contact{
														contacts = Contact#chat_contact.contacts -- maps:keys(Del)
													}}
											end
									end, Data);
merge_game_tab2(trade, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filter(fun
								 (Trade) ->
									 not maps:is_key(Trade#trade.owner, Del)
							 end, Data);
merge_game_tab2(yy_role, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filter(fun
								 (#yy_role{key={_,RoleID}}) ->
									 not maps:is_key(RoleID, Del)
							 end, Data);
merge_game_tab2(mirror, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filter(fun
								 (Mirror) ->
									 not maps:is_key(Mirror#mirror.id, Del)
							 end, Data);
merge_game_tab2(dating, Data) ->
	#mergeing{del_roles=Del} = get_merge_info(),
	lists:filter(fun
								 (Dating) ->
									 not maps:is_key(Dating#dating.id, Del)
							 end, Data);
merge_game_tab2(yy_shop_act, Data) ->
	lists:map(fun
							(YYShopAct1) ->
								#yy_shop_act{act_id=ActID, world_lv=WorldLv1, day=Day1} = YYShopAct1,
								case db:dirty_read(?DB_YY_SHOP_ACT, ActID) of
									[YYShopAct2] ->
										#yy_shop_act{world_lv=WorldLv2, day=Day2} = YYShopAct2,
										YYShopAct1#yy_shop_act{
...
merge_game_tab2(game_misc, Data) ->
	#mergeing{del_roles=Del, merge_to=SUID} = get_merge_info(),
	lists:filtermap(fun
										(#game_misc{key=Key, val=Val}) ->
											Result = case Key of
																 role_id ->
											...
											case Result of
												{true, NewVal} ->
													{true, #game_misc{key=Key, val=NewVal}};
												false ->
													false
											end
									end, Data);
