...
-ifndef(pb_1201_fight).
-define(pb_1201_fight, true).

-define(pb_1201_fight_gpb_version, "4.5.1").

-ifndef('M_FIGHT_ATTACK_TOS_PB_H').
-define('M_FIGHT_ATTACK_TOS_PB_H', true).
-record(m_fight_attack_tos,
        {unit                   :: integer(),       % = 1, 32 bits
         skill                  :: integer(),       % = 2, 32 bits
         dir                    :: float() | integer() | infinity | '-infinity' | nan, % = 3
         defid                  :: non_neg_integer(), % = 4, 32 bits
         coord                  :: pb_1201_fight:p_coord(), % = 5
         seq                    :: integer()        % = 6, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_ATTACK_TOC_PB_H').
-define('M_FIGHT_ATTACK_TOC_PB_H', true).
-record(m_fight_attack_toc,
        {atkid                  :: non_neg_integer(), % = 1, 32 bits
         unit                   :: integer(),       % = 2, 32 bits
         skill                  :: integer(),       % = 3, 32 bits
         cd                     :: non_neg_integer(), % = 4, 32 bits
         level                  :: integer(),       % = 5, 32 bits
         dir                    :: float() | integer() | infinity | '-infinity' | nan, % = 6
         dmgs1 = []             :: [pb_1201_fight:p_damage()] | undefined, % = 7
         combo                  :: integer(),       % = 8, 32 bits
         dmgs2 = []             :: [pb_1201_fight:p_damage()] | undefined, % = 9
         seq                    :: integer(),       % = 10, 32 bits
         coord                  :: pb_1201_fight:p_coord() | undefined % = 11
        }).
-endif.

-ifndef('M_FIGHT_REVIVE_TOS_PB_H').
-define('M_FIGHT_REVIVE_TOS_PB_H', true).
-record(m_fight_revive_tos,
        {type                   :: integer()        % = 1, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_REVIVE_TOC_PB_H').
-define('M_FIGHT_REVIVE_TOC_PB_H', true).
-record(m_fight_revive_toc,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         type                   :: integer(),       % = 2, 32 bits
         dest                   :: pb_1201_fight:p_coord() % = 3
        }).
-endif.

-ifndef('M_FIGHT_PKMODE_TOS_PB_H').
-define('M_FIGHT_PKMODE_TOS_PB_H', true).
-record(m_fight_pkmode_tos,
        {pkmode                 :: integer()        % = 1, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_PKMODE_TOC_PB_H').
-define('M_FIGHT_PKMODE_TOC_PB_H', true).
-record(m_fight_pkmode_toc,
        {pkmode                 :: integer()        % = 1, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_COLLECT_TOS_PB_H').
-define('M_FIGHT_COLLECT_TOS_PB_H', true).
-record(m_fight_collect_tos,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         type                   :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_COLLECT_TOC_PB_H').
-define('M_FIGHT_COLLECT_TOC_PB_H', true).
-record(m_fight_collect_toc,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         type                   :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_PICKUP_TOS_PB_H').
-define('M_FIGHT_PICKUP_TOS_PB_H', true).
-record(m_fight_pickup_tos,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         scene                  :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_PICKUP_TOC_PB_H').
-define('M_FIGHT_PICKUP_TOC_PB_H', true).
-record(m_fight_pickup_toc,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         scene                  :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_DEAD_TOC_PB_H').
-define('M_FIGHT_DEAD_TOC_PB_H', true).
-record(m_fight_dead_toc,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         type                   :: integer(),       % = 2, 32 bits
         who                    :: iolist(),        % = 3
         args = #{}             :: #{iolist() := iolist()} | undefined % = 4
        }).
-endif.

-ifndef('M_FIGHT_NEWBIE_TOS_PB_H').
-define('M_FIGHT_NEWBIE_TOS_PB_H', true).
-record(m_fight_newbie_tos,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         id                     :: integer(),       % = 2, 32 bits
         skill                  :: integer(),       % = 3, 32 bits
         dir                    :: float() | integer() | infinity | '-infinity' | nan, % = 4
         seq                    :: integer()        % = 5, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_AUTOPICK_TOS_PB_H').
-define('M_FIGHT_AUTOPICK_TOS_PB_H', true).
-record(m_fight_autopick_tos,
        {uids = []              :: [non_neg_integer()] | undefined, % = 1, 32 bits
         scene                  :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_AUTOPICK_TOC_PB_H').
-define('M_FIGHT_AUTOPICK_TOC_PB_H', true).
-record(m_fight_autopick_toc,
        {uids = []              :: [non_neg_integer()] | undefined, % = 1, 32 bits
         scene                  :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_DAMAGE_TOC_PB_H').
-define('M_FIGHT_DAMAGE_TOC_PB_H', true).
-record(m_fight_damage_toc,
        {dmgs = []              :: [pb_1201_fight:p_damage()] | undefined, % = 1
         coord                  :: pb_1201_fight:p_coord() | undefined % = 2
        }).
-endif.

-ifndef('M_FIGHT_ENEMIES_TOS_PB_H').
-define('M_FIGHT_ENEMIES_TOS_PB_H', true).
-record(m_fight_enemies_tos,
        {
        }).
-endif.

-ifndef('M_FIGHT_ENEMIES_TOC_PB_H').
-define('M_FIGHT_ENEMIES_TOC_PB_H', true).
-record(m_fight_enemies_toc,
        {enemies = #{}          :: #{integer() := boolean() | 0 | 1} | undefined % = 1
        }).
-endif.

-ifndef('M_FIGHT_ENEMY_TOS_PB_H').
-define('M_FIGHT_ENEMY_TOS_PB_H', true).
-record(m_fight_enemy_tos,
        {suid                   :: integer(),       % = 1, 32 bits
         type                   :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('M_FIGHT_ENEMY_TOC_PB_H').
-define('M_FIGHT_ENEMY_TOC_PB_H', true).
-record(m_fight_enemy_toc,
        {suid                   :: integer(),       % = 1, 32 bits
         type                   :: integer()        % = 2, 32 bits
        }).
-endif.

-ifndef('P_DAMAGE_PB_H').
-define('P_DAMAGE_PB_H', true).
-record(p_damage,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         unit                   :: integer(),       % = 2, 32 bits
         coord                  :: pb_1201_fight:p_coord(), % = 3
         hp                     :: integer(),       % = 4, 32 bits
         type                   :: integer(),       % = 5, 32 bits
         value                  :: integer(),       % = 6, 32 bits
         state                  :: integer()        % = 7, 32 bits
        }).
-endif.
...
-record(p_role_base,
        {id                     :: non_neg_integer(), % = 1, 32 bits
         ...
         figure = #{}           :: #{iolist() := pb_1201_fight:p_aspect()} | undefined, % = 8
         ...
         icon                   :: pb_1201_fight:p_icon() | undefined, % = 17
...
-record(p_item,
        {uid                    :: integer(),       % = 1, 32 bits
         ...
         gender                 :: integer(),       % = 7, 32 bits
         score                  :: integer() | undefined, % = 8, 32 bits
         equip                  :: pb_1201_fight:p_equip() | undefined, % = 9
         pet                    :: pb_1201_fight:p_pet() | undefined, % = 10
...
-record(p_equip,
        {base                   :: pb_1201_fight:p_attr(), % = 1
         rare1                  :: pb_1201_fight:p_attr() | undefined, % = 2
         rare2                  :: pb_1201_fight:p_attr() | undefined, % = 3
         rare3                  :: pb_1201_fight:p_attr() | undefined, % = 4
         marriage               :: pb_1201_fight:p_marriage() | undefined, % = 5
         ...
         refine = []            :: [pb_1201_fight:p_refine()] | undefined, % = 11
         suite = #{}            :: #{integer() := integer()} | undefined, % = 12
         combine = []           :: [pb_1201_fight:p_item()] | undefined % = 13
        }).
-endif.
...
-record(p_marriage,
        {husband_id             :: non_neg_integer() | undefined, % = 1, 32 bits
         ...
         rare                   :: pb_1201_fight:p_attr() | undefined % = 5
        }).
-endif.
...
-record(p_actor,
        {uid                    :: non_neg_integer(), % = 1, 32 bits
         ...
         coord                  :: pb_1201_fight:p_coord(), % = 4
         state                  :: integer(),       % = 5, 32 bits
         stargs = #{}           :: #{integer() := iolist()} | undefined, % = 6
         role                   :: pb_1201_fight:p_role() | undefined, % = 7
         creep                  :: pb_1201_fight:p_creep() | undefined, % = 8
         drop                   :: pb_1201_fight:p_drop() | undefined % = 9
        }).
-endif.
...
-record(p_role,
        {career                 :: integer(),       % = 1, 32 bits
         ...
         figure = #{}           :: #{iolist() := pb_1201_fight:p_aspect()} | undefined, % = 5
         ...
         icon                   :: pb_1201_fight:p_icon() | undefined % = 25
        }).
-endif.
...
-record(p_creep,
        {id                     :: integer(),       % = 1, 32 bits
         ...
         buffs = []             :: [pb_1201_fight:p_buff()] | undefined, % = 6
         dir                    :: integer(),       % = 7, 32 bits
         dest                   :: pb_1201_fight:p_coord(), % = 8
...
-record(p_drop,
        {id                     :: integer(),       % = 1, 32 bits
         ...
         coord                  :: pb_1201_fight:p_coord() | undefined, % = 5
...
-record(p_ranking,
        {base                   :: pb_1201_fight:p_role_base() | undefined, % = 1
